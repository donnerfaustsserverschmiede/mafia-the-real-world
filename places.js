...
(async()=>{
  ...
  const TILE_Z=14;
  const CACHE_KEY='mtrw_world_tiles_v3';
  const CACHE_TTL=1000*60*60*24*7;
  const MAX_LOCAL_TILES=48;
  ...
  async function requestTile(t){
    const key=tileKey(t),cache=readCache(),hit=cache[key];
    if(hit&&Date.now()-hit.time<CACHE_TTL){mergeCache();return true}
    ...
    try{
      ...
      const r=await fetch(TILE_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json','Authorization':'Bearer '+session.access_token,'apikey':typeof SUPABASE_PUBLISHABLE_KEY!=='undefined'?SUPABASE_PUBLISHABLE_KEY:''},body:JSON.stringify({lat:(t.y?0:0),lng:0,z:t.z,x:t.x,y:t.y,tile_key:key})});
      /* The service accepts coordinates. Convert the tile back to its center. */
      if(!r.ok)throw Error('world_tile_'+r.status);
      const j=await r.json();
      const raw=Array.isArray(j.osm_data)?j.osm_data:[];
      cache[key]={time:Date.now(),places:normalize(raw)};writeCache(cache);mergeCache();
      await loadOwned();draw();
      return true;
    }catch(e){console.warn('[MTRW] global world tile',key,e);return false}
    finally{loading.delete(key)}
  }
  /* Request by geographic position; the Edge Function derives the canonical tile. */
  ...
  function prefetchNeighbours(t){const n=2**TILE_Z;const jobs=[];for(let dx=-1;dx<=1;dx++)for(let dy=-1;dy<=1;dy++){if(dx===0&&dy===0)continue;const x=t.x+dx;if(x<0||x>=n)continue;const y=t.y+dy;if(y<0||y>=n)continue;const lon=(x+.5)/n*360-180;const lat=Math.atan(Math.sinh(Math.PI*(1-2*(y+.5)/n)))*180/Math.PI;jobs.push([lat,lon])}setTimeout(()=>jobs.slice(0,2).forEach(([lat,lng])=>requestAt(lat,lng)),250)}
  ...
  if(window.marker){const p=window.marker.getLatLng();requestAt(p.lat,p.lng)}
})();
