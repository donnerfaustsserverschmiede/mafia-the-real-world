...
.res-marker{display:none!important;visibility:hidden!important}.building-marker,.mtrw-stable-building-marker{display:none!important;visibility:hidden!important}
...
function render(){if(!map||!layer)return;const c=map.getCenter(),r0=Math.floor(c.lat/GLAT),c0=Math.floor(c.lng/GLNG),key=`${r0}:${c0}`;if(key===lastKey)return;lastKey=key;layer.clearLayers();for(let r=r0-R;r<=r0+R;r++)for(let c=c0-R;c<=c0+R;c++){const lat=(r+.5)*GLAT,lng=(c+.5)*GLNG;res(r,c).forEach((p,i)=>L.marker([lat+(i?0.00025:-0.00025),lng+(i?0.00032:-0.00032)],{interactive:false,zIndexOffset:1000,icon:L.divIcon({className:'mtrw-resource-marker',html:`<span class="${p}">${icon(p)}</span>`,iconSize:[26,26],iconAnchor:[13,13]})}).addTo(layer))}}
