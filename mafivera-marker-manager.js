...
function render(){if(!map||!layer)return;const center=map.getCenter(),r0=Math.floor(center.lat/GLAT),c0=Math.floor(center.lng/GLNG),key=`${r0}:${c0}:${map.getZoom()}`;if(key===lastKey)return;lastKey=key;layer.clearLayers();const visible=[];for(let r=r0-R;r<=r0+R;r++)for(let c=c0-R;c<=c0+R;c++)visible.push({r,c});for(const q of visible){const lat=(q.r+.5)*GLAT,lng=(q.c+.5)*GLNG,p=baseResourceForTile(q.r,q.c);L.marker([lat,lng],{interactive:false,zIndexOffset:1000,icon:L.divIcon({className:'mtrw-resource-marker',html:'<span class="'+p+'">'+icon(p)+'</span>',iconSize:[30,30],iconAnchor:[15,15]})}).addTo(layer)}}
