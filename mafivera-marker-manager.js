...
function resourceForTile(r,c){
  const n=Math.abs((r*73856093)^(c*19349663))%resources.length;
  return resources[n];
}
function style(){
  if(document.getElementById('mtrwResourceMarkerCSS'))return;
  const s=document.createElement('style');s.id='mtrwResourceMarkerCSS';s.textContent=`
...
/* No building blips from this marker engine. */
...
function renderResources(){
  ...
  for(let r=r0-R;r<=r0+R;r++){
    for(let c=c0-R;c<=c0+R;c++){
      const lat=(r+.5)*GLAT,lng=(c+.5)*GLNG;
      const p=resourceForTile(r,c);
      L.marker([lat,lng],{
        interactive:false,
        zIndexOffset:1000,
        icon:L.divIcon({className:'mtrw-resource-marker',html:`<span class="${p}">${icon(p)}</span>`,iconSize:[30,30],iconAnchor:[15,15]})
      }).addTo(resourceLayer);
    }
  }
}
function boot(){
  ...
  renderResources();
  map.on('moveend',renderResources);
  map.on('zoomend',()=>{lastKey='';renderResources()});
}
