...
async function openProduction(){
 ...
 function calcWeapon(){const r=recipes[weapon?.value];if(!r||!winfo)return;const n=Math.max(1,Math.floor(Number(wqty.value)||1));wqty.value=n;const s=r.seconds*n;winfo.innerHTML=fmt(r.parts*n)+' Waffenteile benötigt · Produktionszeit '+Math.floor(s/60)+' Min. '+(s%60?String(s%60).padStart(2,'0')+' Sek.':'')+' · Verkauf '+fmt(r.value)+' $/Stück · Waffenteilekosten: '+fmt(r.parts*n)}
