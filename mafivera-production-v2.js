...
async function openProduction(){
 ...
 const factory=Number(fq.data?.building_level||0),factoryReady=!!fq.data&&(!fq.data.building_finish_at||new Date(fq.data.building_finish_at)<=new Date()),factoryBoost=Math.min(20,factory)*5;
 ...
 '<div class="production-card"><h3>⚗️ Drogenproduktion</h3><label>Droge</label><select id="mtrwProdDrug">'+drugRows+'</select><label>Menge</label><input id="mtrwProdQty" type="number" min="1" value="1"><div id="mtrwRecipeInfo" class="hint"></div><button id="mtrwProdStart" class="action primary" type="button" '+(used>=slots?'disabled':'')+'>▶️ Produktion starten</button></div>'+
 '<div class="production-card"><h3>🏭 Waffenproduktion</h3><p class="hint">'+(factoryReady?'Waffenfabrik Stufe '+factory+'/20 · +'+factoryBoost+'% Produktionsgeschwindigkeit.':'Eine fertige Waffenfabrik wird benötigt.')+'</p><label>Waffentyp</label><select id="mtrwWeaponType">'+weaponRows+'</select><label>Menge</label><input id="mtrwWeaponQty" type="number" min="1" value="1"><div id="mtrwWeaponInfo" class="hint"></div><button id="mtrwWeaponStart" class="action primary" type="button" '+(used>=slots||!factoryReady?'disabled':'')+'>▶️ Waffenproduktion starten</button></div>'+
 '<h3>Aktive Produktionen · '+used+'/'+slots+'</h3><div class="list">'+(active||'<div class="hint">Keine laufenden Produktionen.</div>')+'</div>');
 const drug=$('mtrwProdDrug'),qty=$('mtrwProdQty'),info=$('mtrwRecipeInfo'),start=$('mtrwProdStart'),weapon=$('mtrwWeaponType'),wqty=$('mtrwWeaponQty'),winfo=$('mtrwWeaponInfo'),wstart=$('mtrwWeaponStart');
 function calcDrug(){const r=recipes[drug?.value];if(!r)return;const n=Math.max(1,Math.floor(Number(qty.value)||1));qty.value=n;const s=r.seconds*n;info.innerHTML=fmt(r.material*n)+' Material benötigt · Produktionszeit '+Math.floor(s/60)+' Min. '+(s%60?String(s%60).padStart(2,'0')+' Sek.':'')+' · Verkauf '+fmt(r.value)+' $/Stück'}
 function calcWeapon(){const r=recipes[weapon?.value];if(!r||!winfo)return;const n=Math.max(1,Math.floor(Number(wqty.value)||1));wqty.value=n;const s=r.seconds*n;winfo.innerHTML=fmt(r.parts*n)+' Waffenteile benötigt · Produktionszeit '+Math.floor(s/60)+' Min. '+(s%60?String(s%60).padStart(2,'0')+' Sek.':'')+' · Verkauf '+fmt(r.value)+' $/Stück · Fabrikbonus +'+factoryBoost+'%'}
 drug?.addEventListener('change',calcDrug);qty?.addEventListener('input',calcDrug);weapon?.addEventListener('change',calcWeapon);wqty?.addEventListener('input',calcWeapon);calcDrug();calcWeapon();
 start?.addEventListener('click',async()=>{try{const n=Math.max(1,Math.floor(Number(qty.value)||1));await rpc('mtrw_start_production',{p_quantity:n,p_drug_type:drug.value});await openProduction();window.mtrwToast?.('Drogenproduktion gestartet.')}catch(e){window.mtrwToast?.(e.message||'Produktion konnte nicht gestartet werden',true)}});
 wstart?.addEventListener('click',async()=>{try{const n=Math.max(1,Math.floor(Number(wqty.value)||1));await rpc('mtrw_start_production',{p_quantity:n,p_drug_type:weapon.value});await openProduction();window.mtrwToast?.('Waffenproduktion gestartet.')}catch(e){window.mtrwToast?.(e.message||'Waffenproduktion konnte nicht gestartet werden',true)}});
