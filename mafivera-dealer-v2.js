...
function draw(d){const m=window.__mtrwMap,p=coords(d);if(!m||!p)return;const same=marker&&last?.id===d.id;if(same){marker.setLatLng(p);marker.setZIndexOffset(100000);return}removeMarker();marker=L.marker(p,{pane:'markerPane',interactive:true,zIndexOffset:100000,keyboard:true,opacity:1,icon:L.divIcon({className:'mtrw-dealer-marker-v4',html:'<div class="mtrw-dealer-pin">🕴️</div><div class="mtrw-dealer-label">DEALER</div>',iconSize:[150,78],iconAnchor:[75,39]})}).addTo(m);marker.setZIndexOffset(100000);marker.bindTooltip('🕴️ Dealer – Angebot öffnen',{direction:'top',offset:[0,-30],sticky:true});marker.on('click',e=>{L.DomEvent.stopPropagation(e);offer(d)});last=d}
