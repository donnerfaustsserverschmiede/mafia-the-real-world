/* MAFIVERA V2 — phone-only runtime
   Authentication UI is owned exclusively by email-auth.js.
   This runtime only enforces the phone-only target and NEVER replaces the login card.
*/
(()=>{
  ...
  const boot=()=>{
    gate();
    window.addEventListener('resize',gate,{passive:true});
    window.addEventListener('orientationchange',()=>setTimeout(gate,100),{passive:true});
  };
