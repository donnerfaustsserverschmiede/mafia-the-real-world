/* MTRW GPS v1.1 - real device location drives the live game position. */
(()=>{
  ...
  function requestRealLocation(){
    ...
    navigator.geolocation.getCurrentPosition(pos=>{const a=Math.round(pos.coords.accuracy||0);setStatus(`📍 Gerätestandort aktiv · Genauigkeit ca. ${a} m`);originalLocate()},e=>setStatus(errorText(e)),{enableHighAccuracy:true,maximumAge:0,timeout:20000});
  }
  ...
  window.mtrwGpsEmit=(lat,lng,accuracy)=>window.dispatchEvent(new CustomEvent('mtrw:gps-position',{detail:{lat,lng,accuracy}}));
})();
