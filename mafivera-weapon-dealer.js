...
function offer(d){
 ...
 const qty=Math.max(0,Number(d.remaining||0)),price=Math.max(0,Number(d.price||0)),name=names[d.weapon_type]||d.weapon_type||'Waffe';
 document.getElementById('drawerTitle').textContent='Waffenhändler';
 document.getElementById('drawerBody').innerHTML=`<div class="dealer-offer"><div class="dealer-big">🏭</div><h3>Waffenhändler-Angebot</h3><p>Ich kaufe <b>${qty}× ${name}</b> für <b>${(qty*price).toLocaleString('de-DE')} $</b>.</p><p class="hint">${price.toLocaleString('de-DE')} $ pro Stück · ${qty} Stück verfügbar</p><button id="weaponDealerAccept" class="action primary">✅ Waffen verkaufen</button><button id="weaponDealerReject" class="action">❌ Angebot ablehnen</button></div>`;
 el.classList.remove('hidden');
 document.getElementById('weaponDealerAccept').onclick=async()=>{try{const result=await rpc('mtrw_sell_weapon_to_dealer',{p_spawn_id:d.id,p_weapon_type:d.weapon_type,p_quantity:qty});if(result?.money!=null){const hud=document.getElementById('hudMoney');if(hud)hud.textContent=Number(result.money).toLocaleString('de-DE');window.__mtrwWeaponDealerLastSale=result;document.dispatchEvent(new CustomEvent('mtrw:balance-updated',{detail:result}))}dismiss();el.classList.add('hidden');toast(`Verkauft. +${Number(result?.earned||qty*price).toLocaleString('de-DE')} $ gutgeschrieben.`)}catch(e){toast(e.message||'Waffenverkauf fehlgeschlagen.',true)}};
...
function draw(d){
 const m=window.__mtrwMap,p=coords(d);if(!m||!p)return;
 if(marker&&last?.id===d.id){marker.setLatLng(p);return}
 removeMarker();
 marker=L.marker(p,{pane:'markerPane',interactive:true,zIndexOffset:100001,icon:L.divIcon({className:'mtrw-weapon-dealer-marker',html:'<div class="mtrw-weapon-dealer-pin">🔫</div><div class="mtrw-weapon-dealer-label">WAFFENHÄNDLER</div>',iconSize:[180,82],iconAnchor:[90,41]})}).addTo(m);
...
function boot(){if(timer)return;tick();timer=setInterval(tick,5000)}
