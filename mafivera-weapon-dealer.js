...
function offer(d){
 ...
 document.getElementById('weaponDealerReject').onclick=async()=>{try{await rpc('mtrw_reject_weapon_dealer',{p_spawn_id:d.id});dismiss();el.classList.add('hidden');toast('Angebot abgelehnt. Der Waffenhändler kommt später wieder.')}catch(e){toast('Angebot konnte nicht abgelehnt werden.',true)}};
}
