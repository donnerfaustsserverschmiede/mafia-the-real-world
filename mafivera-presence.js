...
const mark=async()=>{
  ...
  inFlight=true;
  try{await window.db.rpc('mtrw_presence_heartbeat')}catch(e){}
  finally{inFlight=false}
};
const boot=async()=>{
  if(!window.db)return;
  try{const {data:{user}}=await window.db.auth.getUser();if(!user)return;userId=user.id;await mark();timer=setInterval(mark,INTERVAL);document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')mark()})}catch(e){}
};
