...
function ensureUi(){
  ...
  const s=document.createElement('style');s.textContent=`
    .mtrw-mailbox-btn{position:fixed;top:50%;left:10px;right:auto;transform:translateY(-50%);z-index:3200;width:52px;height:52px;border:1px solid #596575;border-radius:15px;background:#10151eec;color:#fff;font-size:25px;box-shadow:0 8px 24px #0008;cursor:pointer}
    .mtrw-mailbox-btn:active{transform:translateY(-50%) scale(.94)}
...
function bind(){
  document.querySelectorAll('[data-mail-action]').forEach(b=>b.onclick=async()=>{
    ...
    try{
      ...
      if(a==='friend-open'){await window.db.rpc('mtrw_notification_read',{p_id:id});window.mtrwSocialV2?.();setTimeout(()=>document.getElementById('socialRequestsTab')?.click(),80);return}
      ...
      if(a==='offline-open'){await window.db.rpc('mtrw_notification_read',{p_id:id});openMailbox();return}if(a==='trade-accept'){rpc='mtrw_accept_trade';args={p_trade_id:n.action_data.trade_id}}
...
async function start(){
  ensureUi();await load();
  try{
    const rr=await window.db.rpc('mtrw_absence_report_notify');
    if(!rr.error&&rr.data?.has_report){
      const g=rr.data.gains||{};
      const gainText='<div class="mtrw-offline-report"><b>📊 Offline-Bericht</b><div>💵 +'+Number(g.money||0).toLocaleString('de-DE')+' $ &nbsp; 🧱 +'+Number(g.material||0).toLocaleString('de-DE')+' Material &nbsp; 🔩 +'+Number(g.weapon_parts||0).toLocaleString('de-DE')+' Waffenteile &nbsp; ⭐ +'+Number(g.reputation||0).toLocaleString('de-DE')+' Reputation</div><small>Abwesend seit '+fmtDate(rr.data.since)+' · bis '+fmtDate(rr.data.until)+'</small></div>';
