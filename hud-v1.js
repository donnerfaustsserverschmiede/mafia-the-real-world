...
@media(max-width:480px){
 .hud-stats{gap:3px!important}
 .hud-stats span.hud-stat{min-width:54px!important;padding:4px 5px!important}
 .hud-stats span.hud-stat small{font-size:6px!important;letter-spacing:.8px!important}
 .hud-stats span.hud-stat b{font-size:10px!important}
}
`;
...
const setup=()=>{
 ...
 if(!stats||!product||!material)return false;
 const prepare=(el,label,cls)=>{
   const box=el.closest('span');
   if(!box)return;
   box.classList.add('hud-stat',cls||'');
   if(!box.querySelector('.hud-stat-label')){
     const small=document.createElement('small');small.className='hud-stat-label';small.textContent=label;box.insertBefore(small,el);
   }
 };
 prepare(document.getElementById('money'),'GELD','hud-money');
 prepare(material,'MATERIAL','hud-material');
 prepare(product,'DROGEN','hud-drugs');
 prepare(document.getElementById('level'),'LEVEL','hud-level');
 return true;
};
let tries=0;
const timer=setInterval(()=>{if(setup()||++tries>120)clearInterval(timer)},250);
})();