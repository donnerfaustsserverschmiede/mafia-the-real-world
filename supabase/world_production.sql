...
-- This idempotent schema is applied by GitHub Actions.
...
as $$
  select json_build_object('tile_key',tile_key,'bbox',bbox,'osm_data',osm_data,'fetched_at',fetched_at,'expires_at',expires_at)
  from public.world_tiles where tile_key=p_tile_key;
$$;
grant execute on function public.get_world_tile(text) to authenticated;

-- MAFIVERA V1 persistent production jobs
create table if not exists public.mtrw_production_jobs (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references public.profiles(id) on delete cascade,
  quantity integer not null check(quantity>0),
  material_cost integer not null check(material_cost>0),
...
-- MAFIVERA V1 social invitations and reciprocal friendships
...
begin if uid is null then raise exception 'not_authenticated'; end if; code:=lower(encode(gen_random_bytes(9),'hex')); insert into mtrw_social_invites(code,inviter_id) values(code,uid); return json_build_object('success',true,'code',code); end;$$;
