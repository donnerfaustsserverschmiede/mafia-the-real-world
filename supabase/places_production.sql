-- MTRW PRODUCTION: run this once in Supabase SQL Editor.
-- It is safe to run after any previous places.sql / places_v08 / places_v09 migration.

create table if not exists public.game_places (
  id text primary key, osm_type text not null, osm_id bigint not null, name text not null, kind text not null,
  geometry jsonb, center_lat double precision not null, center_lng double precision not null,
  owner_id uuid references public.profiles(id) on delete set null, claimed_at timestamptz,
  claim_lat double precision, claim_lng double precision, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create or replace function public.game_place_tier(p_kind text) returns text language sql immutable as $$
 select case when lower(coalesce(p_kind,'')) in ('city','town') then 'city' when lower(coalesce(p_kind,'')) in ('suburb','quarter','neighbourhood') then 'district' when lower(coalesce(p_kind,'')) in ('village','hamlet') then 'settlement' when lower(coalesce(p_kind,'')) in ('residential','industrial') then 'area' else 'property' end;
$$;
create or replace function public.game_place_influence(p_tier text) returns integer language sql immutable as $$
 select case lower(coalesce(p_tier,'')) when 'city' then 1000 when 'district' then 250 when 'settlement' then 50 when 'area' then 10 else 1 end;
$$;
create or replace function public.game_place_money(p_tier text) returns bigint language sql immutable as $$
 select case lower(coalesce(p_tier,'')) when 'city' then 5000 when 'district' then 1500 when 'settlement' then 500 when 'area' then 200 else 100 end;
$$;
create or replace function public.game_place_rep(p_tier text) returns integer language sql immutable as $$
 select case lower(coalesce(p_tier,'')) when 'city' then 100 when 'district' then 40 when 'settlement' then 20 when 'area' then 10 else 5 end;
$$;
...
declare a jsonb; x double precision; y double precision; xi double precision; yi double precision; xj double precision; yj double precision; inside boolean:=false; first_point boolean:=true; prev_lat double precision; prev_lng double precision; n integer:=0;
begin
...
create or replace function public.claim_place(p_place_id text,p_osm_type text,p_osm_id bigint,p_name text,p_kind text,p_lat double precision,p_lng double precision,p_geometry jsonb)
